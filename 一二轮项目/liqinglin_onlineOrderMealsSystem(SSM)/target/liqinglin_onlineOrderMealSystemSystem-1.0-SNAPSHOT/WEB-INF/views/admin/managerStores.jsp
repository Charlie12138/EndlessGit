				...
				<c:forEach items="${stores }" var="store">
					<tbody>
						<tr>
							...
							<c:if test="${store.status == status1 }">	
								<form action="/ManagerStoreServlet?status=${status3}" method="post">
							...
							<c:if test="${store.status == status2 }">
								<form action="/ManagerStoreServlet?status=${status1}" method="post">
									<input type = "hidden" name = "storeName" value = "${store.storeName}">
					...
					<c:if test="${totalPage > 1 }">
						...
						<form action="/ListAllStoresServlet" method="get" onsubmit="return check2()">
							...
							<tbody>
								...
								<td colspan="9" align="right">
									...
									<c:if test="${pageNum == 1}">
										...
										<a href="/ListAllStoresServlet?pageNum=${pageNum + 1}">下一页</a>
												
									</c:if>
										
								    <c:if test="${pageNum == totalPage }">
										
										<a href="/ListAllStoresServlet?pageNum=${pageNum - 1}">上一页</a>
