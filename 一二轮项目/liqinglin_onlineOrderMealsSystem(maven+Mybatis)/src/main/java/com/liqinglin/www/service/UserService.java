...
public class UserService {
	...
	public int register(User user) {
		...
		if (userDao.usernameIsExist(user.getUsername()) != 0) {
	...
	public int openStroe(Store store) {
		if (userDao.storeNameIsExist(store.getStoreName()) != 0) {
	...
	public int modifyUserInfo(User user) {
		if(userDao.modifyUserInfo(user) == 1) {
			return Contants.SUCCESS_CODE;
		}
		return Contants.FAIL_CODE;
	}
	...
	public int modifyPassword(User user) {
		if(userDao.modifyPassword(user) == 1) {
			return Contants.SUCCESS_CODE;
		}
		return Contants.FAIL_CODE;
	}
